...
{/* GENERATED FILE. Edit registry/swift/lists/PullToRefresh.swift or PullToRefresh.mdx instead. */}

Scrolling container with a pull-to-refresh gesture, a drawn ring indicator and an `async` refresh closure. The piece owns its `ScrollView`, so the overscroll rubber-band is the system's own and the indicator tracks it one to one. The spinner is held until `onRefresh` returns, so a slow reload stays visible and a fast one still reads as a refresh rather than a flash.
...
<Tabs items={["PullToRefresh.swift"]}>
<Tab value="PullToRefresh.swift">

```swift title="PullToRefresh.swift"
import SwiftUI
import UIKit
