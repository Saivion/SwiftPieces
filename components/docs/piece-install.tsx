...
export function PieceInstall({ item }: { item: RegistryIndexEntry }) {
  return (
    <section className="not-prose mt-12" aria-labelledby="install-heading">
      ...
      <p className="mt-2 text-[13.5px] text-muted">Pick one. Run it from the folder that contains your <code className="font-mono text-foreground">.xcodeproj</code> and the files land inside your app.</p>

      <div className="mt-5 grid gap-5">
        <div className="grid gap-2">
          <p className="text-[12.5px] font-semibold text-foreground">Terminal</p>
      ...
      <p className="mt-5 text-[13px] text-muted">Or copy the source above into your app.</p>
      <p className="mt-6 flex items-center gap-2 rounded-[var(--radius)] bg-surface px-4 py-3 text-[13px] text-muted">
