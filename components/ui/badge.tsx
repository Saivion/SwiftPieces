...
export function Badge({ children, tone = "neutral", className }: { children: ReactNode; tone?: "neutral" | "accent" | "outline"; className?: string }) {
  return (
    <span
      className={cn(
        "inline-flex h-6 items-center gap-1.5 rounded-[4px] px-2.5 text-[11px] font-semibold tracking-wide",
...
export function Eyebrow({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <span className={cn("inline-flex h-8 items-center gap-2 rounded-[4px] bg-surface-2 pl-2.5 pr-3.5 text-[12.5px] font-medium text-foreground", className)}>
